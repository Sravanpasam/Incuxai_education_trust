...
// ── AUTH ROUTES (moved to top to guarantee registration) ──────────────────────

// Auth test email — directly calls Resend to verify API key works
...
// Auth: lazy-load controller and JWT modules
let authCtrl = null;
let authJwt = null;
...
// Auth wrapper — lazy-loads modules on first call
function authHandler(handlerName) {
  return async (req, res, next) => {
    ...
    try {
      authCtrl[handlerName](req, res, next);
    } catch (err) {
