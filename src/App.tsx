...
export default function App() {
  ...
  useEffect(() => {
    ...
    w.showPage = (id: string) => {
      ...
      const hideLoginPages = ['ai4all', 'corporate-course'];
      if (loginBtn) {
        loginBtn.style.display = hideLoginPages.includes(id) ? 'none' : '';
      }
      if (id === 'home') {
        setTimeout(() => {
          loadSpline();
        ...
        <div className="header-right">
          ...
          {isAuthenticated ? (
            <>
              ...
              <button className="btn-login" onClick={() => { authLogout(); window.history.replaceState(null, '', '/'); navigate('/', { replace: true }); }} style={{ background: 'rgba(220,38,38,0.8)', border: 'none' }}>
                Sign Out
              </button>
            </>
          ) : null}
